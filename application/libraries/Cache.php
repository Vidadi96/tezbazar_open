...
class Cache
{
	...
	public function delete_by_like($like = null)
	{
		if ($like === null)
		{
			return FALSE;
		...
		$this->_ci->load->helper('directory');
		foreach (glob($this->_path.$like."*.*") as $filename) {
		    unlink($filename);
		}
		// Reset values
		$this->_reset();
	}
	public function delete_model($modelName=null)
	{
		if ($modelName === null)
		{
			return FALSE;
		...
		if (file_exists($this->_path.$modelName))
		delete_files($this->_path.$modelName, TRUE);
