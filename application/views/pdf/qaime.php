...
$html = '
	<table>
        ...
        <u>'.$this->langs->check.' #'.$users->order_number.'</u>
      </td>
      <td width="20%"></td>
      <td width="35%"></td>
			<td width="25%">
  ...
  $html .= '
    ...
    $obj_pdf->SetFont('', 'B');
    $obj_pdf->Cell(15, 10, '#', 'LTRB', 0, 'C', FALSE);
    $obj_pdf->Cell(50, 10, $this->langs->name, 'LTRB', 0, 'C', FALSE);
    $obj_pdf->Cell(30, 10, $this->langs->count, 'LTRB', 0, 'C', FALSE);
    $obj_pdf->Cell(30, 10, $this->langs->price, 'LTRB', 0, 'C', FALSE);
    $obj_pdf->Cell(28, 10, $this->langs->sum, 'LTRB', 1, 'C', FALSE);

    $i=0;
    $total = 0;
    foreach($pdf_list as $row){
    $i++;
    $total = $total + $row->qiymet_teklifi;
    ...
    if($textLength > 228)
      ...
      $rowheight = 14;
        ...
        $obj_pdf->SetFontSize(12);
        $obj_pdf->SetFont('', '');
        $obj_pdf->Cell(15, $rowheight, $i, 'LBR', 0, 'C', FALSE);
        $obj_pdf->MultiCell(50, $rowheight, $row->title." - ".$row->description, 1, '', 0, 0, '', '', true, 0, false, true, 0, 'M', true);
				$obj_pdf->Cell(30, $rowheight, $row->count, 'LBR', 0, 'C', FALSE);
        $obj_pdf->Cell(30, $rowheight, $row->ex_price." azn", 'LBR', 0, 'C', FALSE);
        $obj_pdf->Cell(28, $rowheight, $row->qiymet_teklifi." azn", 'LBR', 1, 'C', FALSE);
    }

    $html2 = '<br><br><br><span> <b>'.$this->langs->total2.':</b> '.$total.' azn</span>';
  ...
  $html .= '
    ...
    if($textLength > 228)
      ...
      $rowheight = 14;
		...
		if($users->discount)
		{
			$total = $total*(100 - $users->discount)/100;
			$html3 = '<br><br><br><span> <b>'.$this->langs->discount_percentage.':</b> '.$users->discount.'%</span>';
	    $obj_pdf->writeHTML($html3, true, false, true, false, '');
			$html2 = '<br><span> <b>'.$this->langs->total2.':</b> '.number_format($total, 2).' azn</span>';
			$obj_pdf->writeHTML($html2, true, false, true, false, '');
		}	else {
		...
		$html4 = '<br><br><br><br><br><br><br><b>Təhvil verdi ________________________ &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;	Təhvil aldı ________________________</b>';
		$obj_pdf->writeHTML($html4, true, false, true, false, '');
