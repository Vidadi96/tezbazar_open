            ...
            <div class="form-group m-form__group row">
                ...
                <select class="form-control m-select2 m_select2_3" id="" name="parent_id">
                  <option value="0">Şəhər</option>
                    <?php foreach ($list as $item) {
                        echo '<option '.return_selected($this->input->post("parent_id"), $item->region_id).' value="'.$item->region_id.'">'.$item->path.'</option>';
                    }
                    ?>
                </select>
              ...
              <div class="col-lg-3">
                <div class="form-group">
                  ...
                  <input type="text" name="name-<?=$lang->lang_id;?>" class="form-control" id="name" value="" />
                </div>
              </div>
              <?php
              if($i==3)
			...
			<div class="col-lg-12">
				...
				<div class="m-portlet m-portlet--head-solid-bg m-portlet--head-sm" m-portlet="true" id="m_portlet_tools_2">
					...
					<div class="m-portlet__body">
						...
						<!--end: Search Form -->
						<!--begin: Datatable -->
						<table delete_url="/posts/delete_cargo/" class="table table-bordered m-table" active_passive_url="/posts/cargo_set_active_passive/">
							...
							<tbody>
								<?php foreach ($list as $item) {
									...
									$btn_active_passive ='<button active_passive="'.$active_passive.'" id="'.$item->region_id.'" type="button" data-container="body" data-skin="dark" data-toggle="m-popover" data-placement="right" data-content="'.$word.'" class="'.$class.' set_active_passive btn m-btn m-btn--icon btn-sm m-btn--icon-only m-btn--pill m-btn--air"></button>';
									...
									<tr>
										...
										<td>
											<a href="/posts/edit_cargo/'.$item->region_id.'" class="btn btn-primary btn-sm m-btn m-btn--icon m-btn--icon-only">
												<i class="fa fa-pencil-alt"></i>
											</button>
										</td>
										<td>
											<button href="#" class="btn btn-danger btn-sm m-btn m-btn--icon m-btn--icon-only delete" rel="'.$item->region_id.'">
