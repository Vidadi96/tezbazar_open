		...
		<div class="row">
			...
			<div class="col-lg-3">
				...
				<div class="m-portlet m-portlet--accent m-portlet--head-solid-bg m-portlet--head-sm" m-portlet="true" id="m_portlet_tools_2">
                  ...
                  <select class="form-control" id="" name="parent_id">
                    ...
                    <?php
                      foreach ($cat_id as $cat) {
												if($this->uri->segment(3)!=$cat->cat_id)
                        echo '<option '.($cat->cat_id==$items[0]["parent_id"]?'selected':'').' value="'.$cat->cat_id.'">'.$cat->name.'</option>';
                      }
                    ?>
                  </select>
								</div>
                  ...
                  <select class="form-control m-select2" id="m_select2_1" name="discount_id">
                    ...
                    <?php
                      foreach ($discount_id as $d) {
                        ...
                        echo '<option '.($d->discount_id==$items[0]->discount_id?'selected':'').' value="'.$d->discount_id.'">'.$cat->discount_name.'</option>';
                      }
                    ?>
                  </select>
								</div>
                  ...
                  <input type="text" name="order_by" value="<?=$items[0]["order_by"];?>" class="form-control m-input" placeholder="">
								</div>
							</div>
								...
								<div class="col-lg-12">
									...
									<?php if($items[0]["icon"]){?>
										<span class="main_thumb">
												<img class="news_img" id="/img/icons/90x90/<?=$items[0]["icon"];?>" style="height:30px !important; border:1px solid #DDD; border-radius: 2px; margin-right: 5px; float: left;" src="/img/icons/90x90/<?=$items[0]["icon"];?>"/>
												<img class="remove_thumb_main" src="/img/icons/16x16/close.png" /><input type="hidden" name="selected_thumb" value="<?=$items[0]["icon"];?>" />
											</span>
								...
								<div class="col-lg-12">
									...
									<div class="m-radio-inline">
										<label class="m-radio m-radio--solid">
											<input type="radio" name="on_menu" <?=($items[0]["on_menu"])?"checked":"";?> value="1">
											Görünsün
											<span></span>
										</label>
										<label class="m-radio m-radio--solid">
											<input type="radio" name="on_menu" <?=(!$items[0]["on_menu"])?"checked":"";?> value="0">
											Görünməsin
											<span></span>
										</label>
									</div>
							...
							<div class="form-group m-form__group row" style="display: none">
								<div class="col-lg-12">
									...
									<div class="m-radio-inline">
										<label class="m-radio m-radio--solid">
											<input type="radio" name="on_home" <?=($items[0]["on_home"])?"checked":"";?> value="1">
											Görünsün
											<span></span>
										</label>
										<label class="m-radio m-radio--solid">
											<input type="radio" name="on_home" <?=(!$items[0]["on_home"])?"checked":"";?> value="0">
											Görünməsin
											<span></span>
										</label>
									</div>
							...
							<div class="form-group m-form__group row">
								<div class="col-lg-12">
									...
									<div class="m-radio-inline">
										<label class="m-radio m-radio--solid">
											<input type="radio" name="active" <?=($items[0]["active"])?"checked":"";?> value="1">
											<?=$this->langs->active; ?>
											<span></span>
										</label>
										<label class="m-radio m-radio--solid">
											<input type="radio" name="active" <?=(!$items[0]["active"])?"checked":"";?> value="0">
											<?=$this->langs->passive; ?>
											<span></span>
							...
							<div class="form-group m-form__group row">
								<div class="col-lg-12">
									<button type="submit" class="btn btn-success btn-block"><i class="fa fa-save"></i> <?=$this->langs->save2; ?></button>
								</div>
							</div>
						</div>
