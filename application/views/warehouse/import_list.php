...
<!-- END: Subheader -->
	<div class="m-content">
        ...
        <?php if(isset($status)): ?>
                ...
                <form action="/warehouse/import_list" method="GET">
                  <!--begin: Search Form -->
  								<div class="row">
                      ...
                      <label><?=$this->langs->import_type; ?></label>
    									<select class="form-control" name="import_type">
    										...
    										<?php foreach($import_type as $row): ?>
    											<option <?=$import_type2 == $row->entry_name_id?'selected':''; ?> value="<?=$row->entry_name_id; ?>"><?=$row->entry_name; ?></option>
    										<?php endforeach; ?>
    									</select>
  									</div>
                    <div class="col-lg-3 col-md-4">
                      <label><?=$this->langs->provider; ?></label>
    									<select class="form-control" name="provider">
    										...
    										<?php foreach($providers as $row): ?>
    											<option <?=$provider_id == $row->id?'selected':''; ?> value="<?=$row->id; ?>"><?=$row->fullname; ?></option>
    										<?php endforeach; ?>
    									</select>
  									</div>
                    <div class="col-lg-3 col-md-4">
                      ...
                      <label><?=$this->langs->entry_date; ?></label>
    									<input type="text" name="import_date" class="form-control m-input date_time_picker" value="<?=$import_date; ?>">
  									</div>
  									<div class="col-lg-3 col-md-4">
  										<label>&nbsp;</label>
  										<br>
                    ...
                    <input type="hidden" id="token" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                        ...
                        <?php foreach ($import_list as $row): ?>
                          <tr>
                            ...
                            <td><?=$row->date_time; ?></td>
                            <td>
                              <a href="/warehouse/edit_import/<?=$row->order_number; ?>" target="_blank" class="btn btn-primary btn-sm m-btn m-btn--icon m-btn--icon-only">
